...
public class OverlayRenderer {
	...
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, LivingEntity entitylivingbaseIn) {
		...
		this.renderName(entitylivingbaseIn, entitylivingbaseIn.getName().getFormattedText(), matrixStackIn, bufferIn, packedLightIn);
	...
	private static float renderStringAtPos(String text, float x, float y, int color, boolean isShadow, Matrix4f matrix, IRenderTypeBuffer buffer, boolean isTransparent, int colorBackgroundIn, int packedLight) {
		...
		for (int i = 0; i < text.length(); ++i) {
			...
			if (c0 == 167 && i + 1 < text.length()) {
				...
				IGlyph iglyph = fontrenderer.font.findGlyph(c0);
				TexturedGlyph texturedglyph = flag && c0 != ' ' ? fontrenderer.font.obfuscate(iglyph) : fontrenderer.font.getGlyph(c0);
		...
		if (!list.isEmpty()) {
			TexturedGlyph texturedglyph1 = fontrenderer.font.getWhiteGlyph();
