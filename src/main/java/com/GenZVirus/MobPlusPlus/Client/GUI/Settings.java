...
public class Settings extends Screen {
	...
	protected void init() {
		super.init();
		resetButton = new Button(mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 242, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 25, 50, 20, "Reset", (x) -> {
			XMLFileJava.resetToDefault();
		});

		save = new Button(mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 187, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 25, 50, 20, "Save", (x) -> {
			XMLFileJava.save();
		});

		save_and_close = new Button(mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 132, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 25, 70, 20, "Save&Close", (x) -> {
			XMLFileJava.save();
			this.onClose();
		});

		close = new Button(mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 57, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 25, 50, 20, "Close", (x) -> {
			this.onClose();
		});

		enable_disable_borders = new Button(mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 311, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 25, 65, 20, OverlayRenderer.BORDER ? "Border On" : "Border Off", (x) -> {
			if (OverlayRenderer.BORDER) {
				OverlayRenderer.BORDER = false;
				enable_disable_borders.setMessage("Border Off");
			} else {
				OverlayRenderer.BORDER = true;
				enable_disable_borders.setMessage("Border On");
			}
		});
		...
		RED = new SlideButton(mc.getMainWindow().getScaledWidth() / 2 - Width / 2 + offsetRED, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 69, "RED", mc.getMainWindow().getScaledWidth() / 2, (x) -> {
		...
		GREEN = new SlideButton(mc.getMainWindow().getScaledWidth() / 2 - Width / 2 + offsetGREEN, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 53, "GREEN", mc.getMainWindow().getScaledWidth() / 2, (x) -> {
		...
		BLUE = new SlideButton(mc.getMainWindow().getScaledWidth() / 2 - Width / 2 + offsetBLUE, mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 37, "BLUE", mc.getMainWindow().getScaledWidth() / 2, (x) -> {
	...
	public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
		...
		AbstractGui.blit(mc.getMainWindow().getScaledWidth() / 2 - Width / 2, mc.getMainWindow().getScaledHeight() / 2 - height / 2, 0, 0, 0, Width, height, height, Width);
		mc.getTextureManager().bindTexture(OverlayRenderer.Background);
		AbstractGui.blit(mc.getMainWindow().getScaledWidth() / 2 - 91, mc.getMainWindow().getScaledHeight() / 2 - 40, 0, 0, 0, 182, 16, 16, 182);
		...
		mc.fontRenderer.drawString(red, mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 10 - mc.fontRenderer.getStringWidth(red), mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 68, 0xFFFF0000);
		mc.fontRenderer.drawString(green, mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 10 - mc.fontRenderer.getStringWidth(green), mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 52, 0xFF00FF00);
		mc.fontRenderer.drawString(blue, mc.getMainWindow().getScaledWidth() / 2 + Width / 2 - 10 - mc.fontRenderer.getStringWidth(blue), mc.getMainWindow().getScaledHeight() / 2 + height / 2 - 36, 0xFF0000FF);
	...
	public void DiscordIcon() {
		...
		mc.fontRenderer.drawString("Mob++", posX + 28, posY + 4, 0xFFFFFFFF);
	}
